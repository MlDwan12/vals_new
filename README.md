...
e2e не запускаются на чистом клоне без `.env`: `src/data-source.ts` валидирует переменные
окружения уже на импорте, до того как `test/support/test-database.ts` подставит адрес поднятого
контейнера. Значения при этом не важны — DB_* всё равно переопределяются, — но переменные должны
существовать. Достаточно `cp .env.example .env` и заполнить `JWT_SECRET`/`JWT_REFRESH_SECRET`
(≥32 символов, разные), `MEILI_MASTER_KEY` (≥16), `BITRIX_WEBHOOK` и `DB_*` любыми строками.

```bash
yarn typecheck   # tsc --noEmit по всему проекту, включая тесты
```

`yarn typecheck` обязателен отдельным шагом: в `tsconfig.json` включён `isolatedModules`, из-за
чего ts-jest транспилирует тесты **без проверки типов** — зелёный `yarn test` не означает, что
типы целы (так и накопился 21 сломанный тип при 274 зелёных тестах).
## Миграции
...
Миграции сознательно не идемпотентны (`ADD VALUE`/`ADD COLUMN`/`CREATE UNIQUE INDEX` без
`IF NOT EXISTS`) — идемпотентные миграции маскируют реальный дрейф схемы, а прод всё равно
наливается с нуля (round-3 review). Если файл миграции переименовали (поменялся timestamp/класс)
после того, как она уже накатана на dev/staging, `migration:run` там упадёт на попытке применить
"новую" миграцию поверх уже применённой схемы: поправить вручную запись в таблице `migrations`
(`name`/`timestamp` на новые) под ту же БД, не накатывать повторно.
...
предпосылка, без сети/транзакции: не всё скопированное обязано быть на месте — падает и
останавливает перенос только если не хватает файла, реально упомянутого в опубликованном контенте,
остальное — просто предупреждение, см. пункт 4), затем одна транзакция на саму БД с полным откатом
при любой ошибке; id и последовательности сохраняются как в источнике.

**Обязательный порядок:**

1. **Бэкап** целевой (`TARGET_*`) БД перед запуском — ETL не проверяет, что целевая БД пуста;
   если в ней уже что-то есть (в частности, если сид уже выполнялся раньше ETL — см. пункт 5),
...
2. Синхронизировать `uploads/media/` старого прода в `uploads/media/` нового сервера (rsync/scp)
   **до** запуска ETL, не после: ETL (шаг 4) сам кладёт в этот же каталог файлы из `image_lib` —
   синхронизация поверх них может затереть одноимённые (rsync по умолчанию перезаписывает
   совпадающие по имени файлы вне зависимости от `--delete`; `--delete` отдельно удаляет то, чего
   нет в источнике, — не используйте его здесь, если не уверены, что в целевом `uploads/media/`
   нет ничего, кроме уже скопированного со старого прода). Это только `media` — `image_lib` ETL
   переносит сам (см. пункт 4), синхронизировать отдельно не нужно.
3. `yarn migration:run` — применить полную схему на **пустую** целевую БД.
4. `yarn migrate:prod-data` — сам перенос (см. переменные окружения ниже). Копирует физические
...
5. `yarn seed:developer` — **только после** ETL, не до. Сид создаёт пользователя с `id=1`; если
...
migrate:prod-data` целиком. Перед копированием скрипт сверяет имена файлов `image_lib` с уже
существующими `media.file_name` в целевой БД и останавливается, если есть пересечение, — на пустой
целевой БД (нормальный повтор после отката) пересечение всегда пусто; непустое означает, что ETL уже
успешно переносил эти файлы раньше и повторный прогон перезаписал бы их на диске.
...
Синхронизация этого каталога — **пункт 2** обязательного порядка выше (до ETL, не после).
...
`yarn lint` запускается с `--fix` — он молча правит файлы, поэтому в CI используется `lint:ci`
(без автоправки). Иначе шаг всегда зелёный на всём, что автоправимо.

## Проверки перед пушем

Те же четыре команды гоняет CI (`.github/workflows/ci.yml`) на каждый пуш:

```bash
yarn typecheck && yarn lint:ci && yarn test && yarn test:e2e --runInBand
```
